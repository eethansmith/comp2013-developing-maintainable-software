...
public class GameMetrics {
    ...
    public GameMetrics(Group root) {
        ...
        this.score = initializeText("SCORE: 0",
                Obstacle.THICKNESS * 4, Obstacle.THICKNESS * 28,
                Color.GOLD, Font.font("Arial Black", 50));

        this.lives = initializeText("LIVES: 3",
                Obstacle.THICKNESS * 20, Obstacle.THICKNESS * 28,
                Color.GOLD, Font.font("Arial Black", 50));

        this.round = initializeText("ROUND: 1",
                Obstacle.THICKNESS * 36, Obstacle.THICKNESS * 28,
                Color.GOLD, Font.font("Arial Black", 50));

        root.getChildren().addAll(score, lives, round);
    }
     ...
     * Updates the lives text with the new lives count.
     *
     * @param newRound The new lives count.
     */
    public void updateRound(int newRound) {
        score.setText("ROUND " + newRound);
    }
    ...
    public void removeTextsFromRoot() {
        root.getChildren().removeAll(score, lives, round);
    }
